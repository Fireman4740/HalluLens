...
class WikiQA:
    def __init__(self, q_generator_path, task, max_workers=50):
        ...
        # prompt
        self.Q_GENERATION_PROMPT = (
            PRECISE_Q_GENERATION_PROMPT
            if task == "precise"
            else LONGFORM_Q_GENERATION_PROMPT
        )
        ...
        try:
            self.encoding = AutoTokenizer.from_pretrained(
                q_generator_path, trust_remote_code=True
            )
        except Exception:
            # Fallback for OpenRouter/OpenAI models not on Hugging Face
            self.encoding = AutoTokenizer.from_pretrained("gpt2")

    def generate_QA_with_doc(
        self,
        title,
        document,
        language="en",
        ...
        for section in sections:
            ...
            while fail_time < self.Q_FAIL_TIME:
                ...
                if q == -1:
                    return []  # when the q generation failed
                a = self.generate_answerability(q, section, language)

                if a == -1:
                    fail_time += 1
                    # if fail_time == 3: assert False
                    if fail_time == self.Q_FAIL_TIME:
                        continue
                    continue
                else:
                    break
            paired_rqas.append({"reference": section, "question": q, "answer": a})
    ...
    def justify_answerability(self, reply):
        if (
            reply.strip().lower() == "unanswerable"
    ...
    def per_bin_generation_batch(self, wiki_data, output_path, N):
        ...
        for i, answer in enumerate(ans_results):
            ...
            if len(QAs) >= N:
                print(
                    "Finished. Filter out {} unanswerable questions.".format(
                        filter_count
                    )
                )
                break
...
def longform_QA_generation_run_batch(
    wiki_input_path,
    N=250,
    q_generator="deepseek/deepseek-v3.2",
    output_path="",
    from_scratch=False,
    low_level=5,
    high_level=10,
    ...
    for bin in range(low_level, high_level):
        ...
        if len(bin_QAs) != per_level_count:
            print(
                f"Warning: expected {per_level_count} questions, got {len(bin_QAs)} for bin {bin}."
            )
        QAs_all.extend(bin_QAs)
